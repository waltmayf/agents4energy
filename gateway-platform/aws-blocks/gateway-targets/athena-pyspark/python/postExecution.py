...
# s3ArtifactsPrefix (defined in sessionSetup.py) is the full S3 key prefix,
# e.g. "files/artifacts/<subdir>/". The frontend's artifact convention (see
# docs/hpc-analytics-agents-epic.md) is "/artifacts/<subdir>/..." — strip the
# "files/artifacts/" root to get the URL-relative piece.
_ARTIFACTS_KEY_ROOT = 'files/artifacts/'
_artifactsUrlPrefix = (
    s3ArtifactsPrefix[len(_ARTIFACTS_KEY_ROOT):]
    if s3ArtifactsPrefix.startswith(_ARTIFACTS_KEY_ROOT)
    else ''
)
                ...
                try:
                    ...
                    def get_full_url(file_path):
                        if file_path.startswith(('http://', 'https://')):
                            return file_path
                        return f"/artifacts/{_artifactsUrlPrefix}{file_path}"
