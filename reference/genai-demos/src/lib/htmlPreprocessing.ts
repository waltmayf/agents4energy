...
export function preprocessContent(content: string, chatSessionId: string): string {
  ...
  const createLoadingPlaceholder = (currentLength: number) => {
    // Cycle dots every 100 chars (1-3 dots) to force the block content to change,
    // defeating Streamdown's memoized Block component deduplication.
    const dotCount = (Math.floor(currentLength / 100) % 3) + 1;
    const dots = '.'.repeat(dotCount);
    return `\n\n> ⏳ *Generating visualization… (${currentLength} chars received${dots})*\n\n`;
  };
  ...
  while ((match = iframeStartRegex.exec(content)) !== null) {
    ...
    if (tagEnd === -1) {
      ...
      console.log(`Detected incomplete iframe during streaming (${partialContent.length} chars), replacing with loading indicator`);
      result += createLoadingPlaceholder(partialContent.length);
    ...
    if (!isComplete) {
      console.log(`Detected incomplete iframe during streaming (${iframeContent.length} chars), replacing with loading indicator`);
      result += createLoadingPlaceholder(iframeContent.length);
  ...
  // Rewrite /artifacts/ iframe src to /file?s3Key= with the chat session prefix
  // The agent uses relative paths like /artifacts/plots/chart.png and we resolve the full S3 key
  result = result.replace(
    /<iframe([^>]*)\ssrc=(["'])\/artifacts\/([^"']+)(\2)([^>]*)>/gi,
    (_match, beforeSrc, quote, relativePath, _closeQuote, afterSrc) => {
      const s3Key = `chatSessionArtifacts/sessionId=${chatSessionId}/${relativePath}`;
      const newSrc = `/file?s3Key=${encodeURIComponent(s3Key)}`;
