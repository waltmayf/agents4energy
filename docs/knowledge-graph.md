...
| Piece | What it does |
|---|---|
| [`gateway-platform/aws-blocks/gateway-targets/graph-traverse/handler.ts`](../gateway-platform/aws-blocks/gateway-targets/graph-traverse/handler.ts) | Gateway-invoked Lambda backing three tools — `TraverseGraph`, `UpsertNode`, `UpsertEdge` — dispatched by `bedrockAgentCoreToolName` in the client context (same pattern as `s3-tools`). Signs its own AppSync requests with SigV4 via `fromNodeProviderChain()`; errors are returned as `{ error: message }` rather than thrown, so the agent sees a readable message instead of a 500. As of #536 this is a standalone copy living in `gateway-platform/` (a separate npm project from `web/`), not `web/amplify/functions/graph-traverse/` — see [`docs/agentic-architecture.md`](./agentic-architecture.md#lambda-backed-gateway-targets-536-moved-into-gateway-platform). |
| [`web/amplify/functions/graph-ingest-lineage/handler.ts`](../web/amplify/functions/graph-ingest-lineage/handler.ts) | DynamoDB Stream handler on the `ChatSession` table (INSERT/MODIFY), still in Amplify (it isn't a gateway target). Skips `MODIFY` events where `lineageSummary` itself didn't change, then calls `ingestLineageSummary`. Failures for one session are logged and skipped rather than failing the whole batch. |
| [`gateway-platform/aws-blocks/gateway-targets/graphTraverse.cdk.ts`](../gateway-platform/aws-blocks/gateway-targets/graphTraverse.cdk.ts) | Reads the Amplify AppSync API's GraphQL url/api id opportunistically from SSM, then registers the traversal Lambda on the AgentCore Gateway as a Lambda-backed MCP target via the generic `LambdaGatewayTarget` construct, with the three tools' JSON-Schema `inputSchema`s in `graphTraverseSchema.ts`. |
...
| `web/amplify/backend.ts` (search `GRAPH-INGEST-LINEAGE`) | Publishes the AppSync GraphQL url/api id to SSM (`/agentcore/<stackName>/graphql_{url,api_id}`) from this Lambda's own sink stack — not `agentStack`, since `agentStack` is already read by the function stack, which is read by the data stack, so an `agentStack → data` edge here would close a `data → function → agent → data` cycle. |

The graph-traverse Lambda's IAM role is granted `appsync:GraphQL` on `Query/fields/*` **and** `Mutation/fields/*` on the API — a query-only grant would leave `UpsertNode`/`UpsertEdge`'s mutations silently rejected by AppSync's authorizer.

---

## Traversal Contract

Core logic — pure, storage-agnostic BFS — lives in [`gateway-platform/aws-blocks/gateway-targets/graph-traverse/graph-traverse-bfs.ts`](../gateway-platform/aws-blocks/gateway-targets/graph-traverse/graph-traverse-bfs.ts) (moved from `web/lib/graph-traverse-bfs.ts` in #536, along with its former unit test — no test currently covers this copy). It takes two injected functions, `fetchEdges` and `fetchNodes`, and issues no queries itself; the sibling [`handler.ts`](../gateway-platform/aws-blocks/gateway-targets/graph-traverse/handler.ts) supplies the real AppSync-backed implementations (`fetchEdgesFromApi`/`fetchNodesFromApi`).
