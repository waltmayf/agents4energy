...
For `@agentcore-claude` runs specifically, the initial comment adds a third follow-along option beyond the chat UI and Live Tail links: a copy-paste `agentcore exec --it` command that attaches a terminal directly to the running session. See [claude-code-agentcore-runtime.md's "Following along in a terminal"](./claude-code-agentcore-runtime.md#following-along-in-a-terminal-issue-203) (issue #203) for the details — it only applies to the ClaudeCode runtime branch, not `MyHarness`.
