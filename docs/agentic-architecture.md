...
### Lambda-backed gateway targets (#536: moved into `gateway-platform/`)

The four Lambda-backed gateway targets — s3-tools (`ApplyDiff`/`ListFiles`/`ReadFile`/`DeleteFile`/`UploadFile`, see [`docs/agent-filesystem.md`](./agent-filesystem.md)), graph-traverse (`TraverseGraph`/`UpsertNode`/`UpsertEdge`, see [`docs/knowledge-graph.md`](./knowledge-graph.md)), athena-pyspark (`SubmitPySpark`/`GetPySparkStatus`/`GetPySparkResults`), and cfd-tools (`SubmitCfdSimulation`/`GetCfdJobStatus`/`GetCfdResults`, `enableHpc`-gated) — used to live in `web/amplify/constructs/*GatewayTarget/` + `web/amplify/backend.ts`. As of #536 the Lambda + `CreateGatewayTarget` custom-resource registration for all four now live in `gateway-platform/aws-blocks/gateway-targets/`, registered against the same-app `Gateway` CDK token (no SSM round-trip needed for that part, since the gateway and its targets are now in the same CDK app).

Each target's tool-implementation code (`handler.ts` + any small sibling lib files) is a **standalone copy** inside its `gateway-targets/<name>/` folder, not a cross-repo reference back into `web/` — the originals depended on `web/lib/*` helpers also used by other `web/` code (the frontend renderer, `storage/resource.ts`, `graph-ingest-lineage`), and `gateway-platform/` is a separate npm project with no dependency on `web/`'s pnpm workspace. Keep both copies in sync by hand if a tool's behavior changes; there's no automated drift check.

Amplify still owns the resources each target's Lambda needs (the `agentWorkspace` S3 bucket, the AppSync GraphQL API, the Athena-for-Spark workgroup + Spark execution role, the PCS/Slurm/FSx-Lustre cluster) — `backend.ts` publishes their names/ARNs to SSM under `/agentcore/<amplifyStackName>/...` (bucket name, GraphQL url/api id, Athena workgroup name/execution role ARN, HPC bucket name/login-node tag), and each `gateway-platform` target construct reads them back opportunistically via `amplifySsm.cdk.ts` — the same synth-time AWS-SDK-`GetParameter`-with-try/catch technique `backend.ts` uses to read the gateway's own coordinates in the other direction (see "Gateway ownership" below). Every target no-ops (creates nothing) when its Amplify-side dependency isn't resolvable yet, so both apps still deploy standalone in either order.

The `*McpServerSeed` custom resources that used to seed a demo `Agent`/`McpServer`/`AgentMcpServer` join row for each tool were dropped entirely in #536 (superseded by the pack-platform slice, #537) — there is currently no seeded demo agent for these tools.
...
The AgentCore Gateway + its `CUSTOM_JWT` authorizer are created by the standalone `gateway-platform/` app (see its own `aws-blocks/agentcore-gateway.cdk.ts`), not by Amplify's `agentStack`. `backend.ts` reads `AGENTCORE_GATEWAY_ID`/`ARN`/`ENDPOINT` from SSM (`/gateway-platform/<stackName>/gateway/{id,arn,endpoint}`) via a synth-time AWS-SDK `GetParameter` call (wrapped in try/catch, defaulting to `''` on any failure) — **not** `StringParameter.valueForStringParameter`/a CloudFormation dynamic reference, which would hard-fail the whole deploy if the parameter doesn't exist yet. By the time `agentStack` resources are declared the value is a plain JS string, so there is no CloudFormation relationship between the two apps. The `ReconcileGatewayAuthorizer` custom resource and the service-webhook Cognito client trust still operate against this gateway id/arn, just sourced from SSM instead of a same-stack construct. The four Lambda-backed gateway targets (s3-tools/graph-traverse/athena-pyspark/cfd-tools) have since moved into `gateway-platform/` itself (#536) — see its own docs/PR for that wiring.
