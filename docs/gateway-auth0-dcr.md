...
**Status: the runbook's core mechanism is validated live; the CDK swap itself is not yet wired.** This document walks through what it would take to point the AgentCore Gateway's inbound `CUSTOM_JWT` authorizer at [Auth0](https://auth0.com/) instead of Cognito, so that MCP clients can **self-register** via OAuth **Dynamic Client Registration (DCR, [RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591))**. The current CDK assets stay on Cognito; treat this as the migration playbook to reach for when you actually need DCR.

> **Validated 2026-08-20 (standalone gateway — the ["Alternative"](#alternative-a-second-dcr-only-gateway) below).** A second AgentCore Gateway (`a4e-deployment-b-auth0`) was stood up by AWS CLI with `authorizer-type CUSTOM_JWT` → `customJWTAuthorizer.discoveryUrl` = the Auth0 tenant discovery URL and `allowedAudience: ["https://a4e-gateway-test/mcp"]` (an Auth0 API identifier), against a tenant with `enable_dynamic_client_registration` on. Confirmed end-to-end: (1) no bearer → **401**; (2) public `POST /oidc/register` DCR self-registration → **201**; (3) a valid Auth0 JWT carrying the gateway audience → **200 past the authorizer**; (4) a *valid* Auth0 JWT for a **different** audience → **403 `insufficient_scope`**; (5) a malformed bearer → **401**. So audience gating (not merely "issued by Auth0") is what the authorizer enforces. What is *not* yet implemented in this repo is the `backend.ts` + reconcile-handler swap of the **primary** gateway (steps 1–4 below); that remains a code change.
...
3. Client **registers itself** via DCR ([RFC 7591](https://datatracker.ietf.org/doc/html/rfc7591)), receiving a fresh `client_id`. ⚠️ **RFC 7592 (client update/delete) is tenant-dependent — do not assume it.** On the tenant tested here (a default dev tenant) the `/oidc/register` response carried **no** `registration_client_uri` / `registration_access_token`, so there is no RFC 7592 management handle: a registered client can't be updated or deleted through the DCR API, only via the Management API by an admin. The outbound DCR code (#449) treats those fields as optional and its teardown DELETE is best-effort for exactly this reason.
4. Client runs a **PKCE authorization-code flow** against Auth0, requesting the gateway API audience. ⚠️ This must be the authorization-code (user-login) flow, **not** `client_credentials`: Auth0 grants DCR-registered ("third-party") clients only the `authorization_code` grant, and a `client_credentials` token request from such a client is rejected with `unauthorized_client`. That is by design and matches the intended pattern — a *user* logs in and consents before the MCP server is reachable. (The live validation above minted its test token from a separate first-party **M2M** app + client-grant purely to exercise the gateway authorizer; the same JWT shape is what the PKCE flow yields, and the gateway can't tell the two apart.)
5. Client presents the resulting access token to the gateway; the authorizer validates it on audience + signature. Because the client registered *itself*, no admin ever touched `allowedClients`. **Verified:** correct-audience JWT → 200; wrong-audience JWT → 403.
