  ...
  if (repo) {
    appendParts.push(
      ...
      'Before you mark the PR ready for review (`gh pr ready <your-branch>`), verify the change builds: run `pnpm install` at the REPOSITORY ROOT (this is a pnpm workspace with one root lockfile — do NOT run it inside web/), then `cd web && npx tsc --noEmit` and make it pass. Leave the PR as a draft while the type check fails, and do not claim it passed unless you actually ran it and it did.',
        'PERSIST TOOLS YOU INSTALL. The container root filesystem (including /usr/local/bin and your HOME) is WIPED between turns and whenever this session is suspended — only `/mnt/workspace` persists. If you install a CLI tool that a later step (e.g. a monitor check) will need, install it under `/mnt/workspace/bin` and reference it by absolute path. Example for the AWS CLI: `curl -fsSL https://awscli.amazonaws.com/awscli-exe-linux-$(uname -m).zip -o /tmp/awscliv2.zip && (cd /tmp && python3 -c "import zipfile;zipfile.ZipFile(\'awscliv2.zip\').extractall()") && /tmp/aws/install --install-dir /mnt/workspace/aws-cli --bin-dir /mnt/workspace/bin --update`. Then invoke it as `/mnt/workspace/bin/aws ...`.',
        'Your `checkCommand` runs in a FRESH container each tick — nothing from this turn\'s environment survives except files under `/mnt/workspace`, and the mount is NOT on PATH. So the checkCommand MUST invoke any tool by absolute path (e.g. `/mnt/workspace/bin/aws`) or prepend PATH inline (`export PATH=/mnt/workspace/bin:$PATH; ...`). Do NOT assume `aws`, `gh`, `node`, etc. are on PATH or even present. Exit 0 = condition met (re-invoke me); non-zero = keep waiting.',
      issueNumber
