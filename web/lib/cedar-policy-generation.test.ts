...
// New test for validationMode based on effect
test('validationMode is IGNORE_ALL_FINDINGS for DENY grants and FAIL_ON_ANY_FINDINGS for ALLOW grants', () => {
  const denyPolicy = generateCedarPolicies([grant({ effect: 'DENY' })], TEST_GATEWAY_ARN)[0];
  const allowPolicy = generateCedarPolicies([grant()], TEST_GATEWAY_ARN)[0];
  assert.equal(denyPolicy.validationMode, 'IGNORE_ALL_FINDINGS');
  assert.equal(allowPolicy.validationMode, 'FAIL_ON_ANY_FINDINGS');
});
