...
import { friendlyHarnessError, friendlyChatHarnessError } from './harness-error-message.ts';
...
test('friendlyChatHarnessError adds a new-session suggestion for context overflow', () => {
  const cause = "Input length (272468) exceeds model's maximum context length (131072).";
  const out = friendlyChatHarnessError(cause);
  assert.ok(out);
  assert.match(out, /context window overflowed/);
  assert.match(out, /start a new chat session/i);
});

test('friendlyChatHarnessError returns null for an unrecognized cause', () => {
  assert.equal(friendlyChatHarnessError('Some other transient Bedrock error'), null);
});
