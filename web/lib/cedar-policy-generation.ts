...
export function generateCedarPolicies(grants: ToolGrantInput[], gatewayArn: string): CedarPolicySpec[] {
  return grants
    ...
    .map((grant) => ({
      ...
      // A DENY grant becomes a Cedar `forbid` whose scope deliberately denies
      // its entire (principal, action, resource) combination — exactly what the
      // admin asked for. AgentCore's analyzer flags that as an "Overly
      // Restrictive" *finding*, so under FAIL_ON_ANY_FINDINGS the forbid policy
      // is rejected (CREATE_FAILED) and the DENY silently never enforces (#368).
      // Ignore findings for forbid policies; keep strict validation for permits,
      // where a finding is a genuine mistake.
