...
export const handler = async (input: PrepareInput): Promise<PrepareOutput> => {
  ...
  if (repo) {
    effectivePrompt = [
      ...
      '',
      'DELIVER INCREMENTALLY — do not save all your work for the end. Your turn can be cut off before you finish, and any work that is only in the container (not pushed) is LOST. So:',
      ...
      `CONFIRM the PR actually exists and capture its real URL: gh pr list --repo ${repo} --head <your-branch> --state open --json url --jq '.[0].url'`,
