...
const DEMO_AGENT_SLUG = 'analytics-pyspark-demo';
const DEMO_AGENT_NAME = 'Analytics PySpark Demo';
const DEMO_MCP_SERVER_NAME = 'Athena PySpark Tools';
  ...
  const created = await signedGraphqlRequest(
    ...
    }`,
    {
      input: {
        ...
        description: 'Demo agent proving the Athena PySpark submit/poll/results tool (issue #501) end-to-end.',
        enabled: true,
      },
    },
  );
...
async function findOrCreateMcpServer(
  ...
  // old McpServer row instead of updating it in place.
  const existing = await signedGraphqlRequest(
    url,
    region,
  ...
  const created = await signedGraphqlRequest(
    ...
    }`,
    {
      input: {
        ...
        url: gatewayEndpoint,
        gatewayTargetId,
        description: 'AgentCore Gateway exposing the Athena-for-Spark PySpark submit/poll/results tools (issue #501).',
...
export const handler = async (
  ...
  // No-op on Delete: this only seeds demo rows to prove the wiring path, and a
  // ChatSession may already reference the demo agent — deleting it out from
  // under an in-progress chat is worse than leaving an idle demo row behind.
