...
async function findOrCreateMcpServer(
  ...
  if (existingItem) {
    // Backfill gatewayTargetId on redeploy (issue #489, same as #359) even
    // when the url is unchanged, so pre-existing rows created before this
    // field was wired up pick it up.
