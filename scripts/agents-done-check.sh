...
# `agent-working`); non-zero otherwise (including on any curl/network error —
# fail toward "not done yet" rather than falsely reporting completion).
#
...
response="$(curl -sf \
  ...
  "${auth_header[@]}" \
  "https://api.github.com/repos/${REPO}/issues?labels=${AGENTS_WORK_LABEL}&state=open&per_page=1")"

# An empty issues array means no open issue carries the label — workers done.
if grep -q '"number"' <<<"$response"; then
  exit 1
fi
exit 0
