...
# Normalise: replace slashes with dashes, lowercase, then slug = first 8 chars +
# '-' + first 5 hex chars of the full name's sha1 (14 chars total, under the
# ampx --identifier limit of 15). A blind truncate-to-14 collided whenever two
# branches shared a 14-char prefix (#400) — the hash suffix makes that
# vanishingly unlikely even for near-identical branch names.
BRANCH_LOWER_DASHED="$(echo "$BRANCH" | tr '/' '-' | tr '[:upper:]' '[:lower:]')"
BRANCH_HASH="$(printf '%s' "$BRANCH_LOWER_DASHED" | sha1sum | cut -c1-5)"
export BRANCH_SLUG="$(echo "$BRANCH_LOWER_DASHED" | cut -c1-8)-$BRANCH_HASH"
...
# hashed, and truncated to 14 chars — the same value passed to
# `ampx sandbox --identifier`).
