...
try {
  ...
  console.warn('extract-deployment-info: could not read amplify_outputs.json — skipping AppSync info');
}

// Resolve the AppSync API ID, preferring the sidecar file written during deploy,
// falling back to a CloudFormation/AppSync API list lookup.
let appsyncApiId = '';
if (amplifyOutputs) {
  ...
  try {
    ...
    try {
      const cfnApiRaw = execSync(`aws appsync list-graphql-apis --region ${appsyncRegion} --output json`, { encoding: 'utf8' });
