...
**AgentCore resources**: Config lives as typed TypeScript in `web/amplify/agentcore/agentcore.config.ts` (memories, runtimes, policy engines) and is fed to the `AgentCoreApplication` L3 CDK construct (`web/amplify/constructs/agentCoreApplication.ts`), which `backend.ts` instantiates directly in its own `backend.createStack(...)` (`agentStack`). This provisions a Bedrock AgentCore Harness (`MyHarness`, prompt at `web/amplify/agentcore/MyHarness/system-prompt.md`) backed by `openai.gpt-oss-120b`, persistent memory (`MyHarnessMemory` with SEMANTIC, USER_PREFERENCE, SUMMARIZATION, and EPISODIC strategies), the built-in `agentcore_browser` tool, and AgentCore Runtimes (`ClaudeCode`, `AguiAgent`) built via CodeBuild → ECR → CfnRuntime. (The `agentcore_code_interpreter` sandbox was removed — see #191 — so the agent runs shell commands in the harness runtime session.) The MCP **Gateway** that validates Cognito JWTs is no longer provisioned here — see "Gateway ownership (#535)" in [docs/agentic-architecture.md](docs/agentic-architecture.md#gateway-ownership-535).
...
- The AgentCore Memory ARN is **derived** from the `AgentCoreApplication` construct in `web/amplify/backend.ts` (`agentCoreApp.memoryArn(...)`) and published to SSM Parameter Store under `/agentcore/<stackName>/…` to avoid cross-stack CloudFormation export cycles — it is **not** hardcoded, so no manual update is needed after an AgentCore redeploy. The **Gateway** is no longer created here (#535) — it's created by the standalone `gateway-platform/` app, and `backend.ts` reads its id/arn/endpoint from `/gateway-platform/<stackName>/gateway/{id,arn,endpoint}` in SSM via an AWS-SDK `GetParameter` call made at synth time (module load, before `app.synth()`), wrapped in try/catch and defaulting to `''` on any failure — **not** a CloudFormation dynamic reference (`StringParameter.valueForStringParameter`/`{{resolve:ssm:...}}`), which would hard-fail the whole deploy if the parameter doesn't exist yet. By the time `agentStack` resources are declared, the value is a real JS string, not a CDK token, so every `if (AGENTCORE_GATEWAY_ID)` gate actually skips gateway-dependent constructs when gateway-platform hasn't published yet — see [docs/agentic-architecture.md](docs/agentic-architecture.md#gateway-ownership-535).
